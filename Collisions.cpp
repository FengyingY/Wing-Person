...
bool Collisions::circle_circle_collision(Shapes::Circle c1, Shapes::Circle c2) {
	...
	float center_dist = std::sqrt(std::pow(c1.center.x - c2.center.x, 2.f) + std::pow(c1.center.y - c2.center.y, 2.f));
...
glm::vec2 Collisions::circle_circle_collision(Shapes::Circle c1, Shapes::Circle c2, int axis) {
	...
	if (center_dist < c1.radius + c2.radius) {
		...
		if (axis == 0) {
			...
			desired_dist = std::sqrt(std::pow(radius_dist, 2.f) - std::pow(y_dist, 2.f)) * avg_diff.x;
		}
		else if (axis == 1) {
			avg_diff.x = 0.0f;
			if (avg_diff.y < 0.0f)
			...
			desired_dist = std::sqrt(std::pow(radius_dist, 2.f) - std::pow(x_dist, 2.f)) * avg_diff.y;
		}
		else {
		...
		if (avg_diff.x == 0.0f) {
			...
			vector_from_c2_x = (desired_dist * ratio_avg_diff) / sqrt(1.0f + std::pow(ratio_avg_diff, 2.f));
	...
	for (int i = 0; i < 4; i++) {
		if (r1.corners[i].x < r2.corners[0].x && r1.corners[i].x > r2.corners[1].x &&
			...
			if (avg_diff.x == 0.0f) {
				...
				float ratio_avg_diff = avg_diff.y / avg_diff.x;
				vector_from_r2_x = (desired_dist * ratio_avg_diff) / sqrt(1.0f + std::pow(ratio_avg_diff, 2.f));
...
bool Collisions::circle_rectangle_collision(Shapes::Circle c, Shapes::Rectangle r) {
	...
	float center_dist = std::sqrt(std::pow(c.center.x - r.center.x, 2.f) + std::pow(c.center.y - r.center.y, 2.f));
...
bool Collisions::circle_triangle_collision(Shapes::Circle c, Shapes::Triangle t) {
	...
	for (glm::vec2 point : t.points) {
		float dist = std::sqrt(std::pow(c.center.x - point.x, 2.f) + std::pow(c.center.y - point.y, 2.f));
		if (dist < c.radius)
			return true;
	}
	...
	for (glm::vec2 intersect : intersections) {
		...
		if (center_dist < c.radius + intersect_dist) {
			...
			if (avg_diff.x == 0.0f) {
				...
				float ratio_avg_diff = avg_diff.y / avg_diff.x;
				vector_from_r_x = (desired_dist * ratio_avg_diff) / sqrt(1.0f + std::pow(ratio_avg_diff, 2.f));
				vector_from_r_y = vector_from_r_x / ratio_avg_diff;
			}
			glm::vec2 vector_from_r = glm::vec2(vector_from_r_x, vector_from_r_y);
	...
	for (glm::vec2 intersect : intersections) {
		...
		if (center_dist < c.radius + intersect_dist) {
			...
			if (avg_diff.x == 0.0f) {
				...
				float ratio_avg_diff = avg_diff.y / avg_diff.x;
				vector_from_c_x = (desired_dist * ratio_avg_diff) / sqrt(1.0f + std::pow(ratio_avg_diff, 2.f));
				vector_from_c_y = vector_from_c_x / ratio_avg_diff;
			}
			glm::vec2 vector_from_c = glm::vec2(vector_from_c_x, vector_from_c_y);
...
bool Collisions::point_in_line_segment(glm::vec2 point, glm::vec2 line_p1, glm::vec2 line_p2) {
	...
	float point_p1_dist = std::sqrt(std::pow(point.x - line_p1.x, 2.f) + std::pow(point.y - line_p1.y, 2.f));
	float point_p2_dist = std::sqrt(std::pow(point.x - line_p2.x, 2.f) + std::pow(point.y - line_p2.y, 2.f));
	float line_dist = std::sqrt(std::pow(line_p1.x - line_p2.x, 2.f) + std::pow(line_p1.y - line_p2.y, 2.f));
