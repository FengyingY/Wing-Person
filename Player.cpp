...
Player::Player(Shapes::Rectangle bounds, Input* left, Input* right, Input* jump)
    : bounding_box(bounds), _left(left), _right(right), _jump(jump)
{
...
void Player::draw(glm::uvec2 const &drawable_size) {
	...
	draw_rectangle(glm::vec2(bounding_box.center), glm::vec2(bounding_box.width, bounding_box.height), glm::u8vec4(0xe3, 0x00, 0x00, 0xff));
