...
PuzzleStage::PuzzleStage() {
  ...
  add_player(Player(Shapes::Rectangle(glm::vec2(200, 200), 50, 80), SDLK_a, SDLK_d, SDLK_w));
  add_player(Player(Shapes::Rectangle(glm::vec2(600, 200), 50, 80), SDLK_LEFT, SDLK_RIGHT, SDLK_UP));
...
PuzzleStage::tick(float elapsed) {
  ...
  for (auto& player : _players) {
    ...
    if(player._cur_jump_time < player._input_jump_time) {
      ...
      if(player._cur_jump_time >= player.input_jump_time) {
